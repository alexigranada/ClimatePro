      ...
      <div className={`layer-control ${isExpanded ? 'expanded' : ''}`}>
        ...
        {isExpanded && (
          <div className="layer-details">
            ...
            <p>Proyección desde 2024 a 2035 (3H)</p>
  ...
  return (
    <div className="map-container">
      <div className="map-wrap">
        ...
        <button className="time-button" onClick={() => changeTime('backward')}>-3h</button>
        <div className="current-time">
          <span>{formatDateTime(currentTime)}</span>
        </div>
        <button className="time-button" onClick={() => changeTime('forward')}>+3h</button>
        </div>
        </div>
      </div>
    </div>
